...
mcp = FastMCP(name="FoncierMCP", instructions="Tu es FoncierMCP, assistant analyse fonciere France. Commence toujours par analyze_parcel pour une analyse complete.")
...
async def auth_middleware(request: Request, call_next):
    ...
    if not path.startswith("/mcp") or any(path.startswith(e) for e in ["/health","/webhook","/api/demo"]):
...
async def index():
    return HTMLResponse(open("static/index.html").read() if os.path.exists("static/index.html") else "<h1>FoncierMCP</h1><p><a href=/health>Health</a></p>")
